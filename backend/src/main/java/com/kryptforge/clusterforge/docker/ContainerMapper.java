...
public final class ContainerMapper {
	...
	public static ContainerDetail toDetail(InspectContainerResponse resp) {
			...
			ports = bindings.entrySet().stream()
					.flatMap(e -> {
						...
						return Arrays.stream(binds).map(b -> {
							String hostIp = (b.getHostIp() != null && !b.getHostIp().isBlank()) ? b.getHostIp()
									: "0.0.0.0";
		...
		return new ContainerDetail(
				...
				resp.getState() != null ? resp.getState().getStatus() : null,
				resp.getConfig() != null && resp.getConfig().getLabels() != null ? resp.getConfig().getLabels()
						: Map.of(),
				mounts,
				networks,
				ports);
	...
	public static ContainerStats toStats(String containerId, Statistics s, Instant startedAt) {
		// CPU %
		...
		try {
			...
			} else if (s.getCpuStats() != null && s.getCpuStats().getCpuUsage() != null
		...
		if (s.getMemoryStats() != null) {
			if (s.getMemoryStats().getUsage() != null)
				memUsage = s.getMemoryStats().getUsage();
			if (s.getMemoryStats().getLimit() != null)
				memLimit = s.getMemoryStats().getLimit();
			if (memLimit > 0) {
		...
		if (s.getNetworks() != null) {
			for (var e : s.getNetworks().entrySet()) {
				if (e.getValue() != null) {
					if (e.getValue().getRxBytes() != null)
						rx += e.getValue().getRxBytes();
					if (e.getValue().getTxBytes() != null)
						tx += e.getValue().getTxBytes();
				}
			}
		}
		...
		if (s.getBlkioStats() != null && s.getBlkioStats().getIoServiceBytesRecursive() != null) {
			...
			for (var entry : list) {
				if (entry == null || entry.getOp() == null || entry.getValue() == null)
					continue;
		...
		return new ContainerStats(
				...
				blkRead, blkWrite,
				pids,
				calculateUptime(s.getRead(), startedAt));
	}

	/**
	 * Calcula o tempo de atividade (uptime) do container em segundos.
	 * Baseado na diferença entre o timestamp de leitura das métricas e o timestamp
	 * de início do container.
	 * 
	 * @param readTimeStr Timestamp ISO-8601 da leitura das métricas
	 * @param startedAt   Instant de quando o container foi iniciado
	 * @return Uptime em segundos, ou 0 se não foi possível calcular
	 */
	private static Long calculateUptime(String readTimeStr, Instant startedAt) {
		if (startedAt == null || readTimeStr == null)
			return 0L;
		try {
			Instant readTime = Instant.parse(readTimeStr);
			long seconds = Duration.between(startedAt, readTime).getSeconds();
			return seconds > 0 ? seconds : 0L;
		} catch (Exception e) {
			log.trace("Erro ao calcular uptime: readTime={}, startedAt={}, erro={}",
					readTimeStr, startedAt, e.getMessage());
			return 0L;
		}
	}
